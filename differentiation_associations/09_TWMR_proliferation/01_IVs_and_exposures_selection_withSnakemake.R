...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!    Important    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
################################################################################
# This code was originally created to run jobs in a Snakemake workflow but 
# wasn't used in the end. See 01_IVs_and_exposures_selection.R. 
################################################################################
...
focal_gene = as.character(args[1]) 
...
find_lead_coloc_variant <- function(focal_gene){
...
gene_pair_cor <- function(initial_genes_in_model){
  ...
  for(i in 1:length(initial_genes_in_model)){
    for(j in 1:length(initial_genes_in_model)){
      ...
      gene1_eQTLs <- nominal_eqtl_results %>% dplyr::select(gene, gene_symbol, variant_id, pval_nominal, slope) %>% filter(gene_symbol == gene1)
      ## all cis-eQTLs for gene2
      gene2_eQTLs <- nominal_eqtl_results %>% dplyr::select(gene, gene_symbol, variant_id, pval_nominal, slope) %>% filter(gene_symbol == gene2)
...
select_IVs_and_exposures <- function(focal_gene){
  ...
  if(!focal_gene %in% colnames(final_model)){
    message(paste0("Focal gene ", focal_gene, " not included in the final model."))
    stop()
  }
  
  if(!setequal(colnames(final_model)[-1], uncorr_eGenes)){
    message(paste0("Not all uncorrelated genes for ", focal_gene, " included in the final model."))
    stop()
  }
...
# E <- select_IVs_and_exposures(focal_gene = "VANGL1")
# [1] "Final model for VANGL1: 0 uncorrelated eGenes as exposures and 1 independent cis-eQTLs as IVs" 
# E <- select_IVs_and_exposures(focal_gene = "ROCK1")
# [1] "Final model for ROCK1: 0 uncorrelated eGenes as exposures and 1 independent cis-eQTLs as IVs"

# * genes with lead coloc variant as unique independent cis-eQTL: 
# E <- select_IVs_and_exposures(focal_gene = "OTOA")
# [1] "Final model for OTOA: 1 uncorrelated eGenes as exposures and 1 independent cis-eQTLs as IVs"
# E <- select_IVs_and_exposures(focal_gene = "LY86")
# [1] "Final model for LY86: 0 uncorrelated eGenes as exposures and 4 independent cis-eQTLs as IVs"

# * gene with 1 independent cis-eQTL with multiple eGenes with signif eQTLs
# E <- select_IVs_and_exposures(focal_gene = "A2M")
# [1] "Final model for A2M: 2 uncorrelated eGenes as exposures and 24 independent cis-eQTLs as IVs"

# * gene with 1 independent cis-eQTL with multiple eGenes with 0 signif eQTLs
# E <- select_IVs_and_exposures(focal_gene = "SAMD11")
# [1] "Final model for SAMD11: 4 uncorrelated eGenes as exposures and 1 independent cis-eQTLs as IVs"

# * gene with 2 independent cis-eQTLs with 1 eGene only (the focal gene)
# E <- select_IVs_and_exposures(focal_gene = "APBB2")
# [1] "Final model for APBB2: 0 uncorrelated eGenes as exposures and 2 independent cis-eQTLs as IVs"

# * gene with 2 (initial) independent cis-eQTLs with multiple eGenes with signif eQTLs
# E <- select_IVs_and_exposures(focal_gene = "BLOC1S3")
# [1] "Final model for BLOC1S3: 8 uncorrelated eGenes as exposures and 21 independent cis-eQTLs as IVs"

# * gene with 2 independent cis-eQTLs with multiple eGenes with 0 signif eQTLs
# E <- select_IVs_and_exposures(focal_gene = "BCKDK")
# [1] "Final model for BCKDK: 2 uncorrelated eGenes as exposures and 2 independent cis-eQTLs as IVs"
...
## Find model for focal gene (E: IVs x eGenes)
if(!focal_gene %in% nominal_eqtl_results$gene_symbol){
  message(paste0(focal_gene, " without cis-eQTLs."))
  stop()
} 

E = select_IVs_and_exposures(focal_gene) 
...
if(any(rownames(C) != rownames(E)) | any(colnames(C) != rownames(E))){
  message(paste0("Not all IVs for ", focal_gene, " are present in C."))
  stop()
}
if(!isSymmetric(C)){
  message(paste0("C for ", focal_gene, " is not symmetric."))
  stop()
}
if(any(! diag(C) == 1)){
  message(paste0("C for ", focal_gene, " has not all 1s on the diagonal."))
  stop()
} 
