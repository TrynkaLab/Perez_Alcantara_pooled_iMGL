...
line_prop_changes_old_vs_young_premac = w_estimates_adjusted %>%
  ...
  dplyr::reframe(
    min_age = min(preMAC_age),
    max_age = max(preMAC_age),
    age_difference = max(preMAC_age) - min(preMAC_age),
    
    log_fraction_mean = log(
      prop_adjusted_mean[preMAC_age == max(preMAC_age)] /
        prop_adjusted_mean[preMAC_age == min(preMAC_age)]
    ),
...
summary(line_prop_changes_old_vs_young_premac$min_age)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 28.00   35.00   36.00   34.92   36.00   36.00 
summary(line_prop_changes_old_vs_young_premac$max_age)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 46.00   49.00   50.00   54.92   54.00  109.00 
summary(line_prop_changes_old_vs_young_premac$age_difference)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 10      13      15      20      18      80 
### write
