...
# line_prop_changes_path = "../../data/results/1.2.scale_proliferation/line_prop_changes_old_vs_young_premac.csv"
    ...
    # correct fr treatment
      ...
      meta_subset = full_pheno_info %>%
        ...
        dplyr::mutate(sex_numeric = case_when(Sex == "Male" ~ 1, # female as reference (0)
                                              .default = 0)) %>%
        dplyr::group_by(line) %>%
        # summarising per line, across pools, types and treatments
        ...
        dplyr::select(line,scaled_log_fraction_pool,prop_unadjusted_min_value_mean,sex_numeric, genotypePC1, genotypePC2) %>%
        distinct()
      ...
      if(dim(vars_within_mut_type)[1]<2 | check_min_lines == FALSE  ){
        ...
        if(chr !="chrX"){
          ...
          obj=SKAT_Null_Model_ChrX(y.c ~ sex + prop_unadjusted_min_value_mean + genotypePC1 + genotypePC2, 
                                   out_type="C",
      ...
      meta_subset = full_pheno_info %>%
        ...
        dplyr::mutate(sex_numeric = case_when(Sex == "Male" ~ 1, # female as reference (0)
                                              .default = 0)) %>%
        dplyr::group_by(line) %>%
        # summarising per line across all pools
        dplyr::reframe(scaled_log_fraction_pool = mean(scaled_log_fraction),
                       prop_unadjusted_min_value_mean = mean(prop_unadjusted_min_value),
                       sex_numeric = sex_numeric, genotypePC1= genotypePC1, genotypePC2 =genotypePC2) %>%
        dplyr::select(line,scaled_log_fraction_pool,prop_unadjusted_min_value_mean,sex_numeric, genotypePC1, genotypePC2) %>%
