...
pseudobulk = pseudobulk[,!discarded]
metadata = metadata[!discarded,]
length(unique(metadata$line)) # 149/250, 73%
...
rbe = limma::removeBatchEffect(pseudobulk,
                               batch = metadata$pool,
                               group = metadata$treatment) # experimental conditions to preserve other than batch
...
transposed = v2$E %>%
  ...
  dplyr::filter(gene %in% c("BIN1","TREM2")) %>%
...
transposed %>%
  ggplot(aes(x = pool,y = voom_counts, colour = Sex)) +
  geom_point() + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 90,hjust = 1, vjust = 0.5)) + 
  facet_wrap(vars(gene))

# drop in TREM2 expression in pool9
## do the same with scaled expression before eQTL