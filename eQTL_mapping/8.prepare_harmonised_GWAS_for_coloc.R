...
outDir = "../../data/results/8.colocalisation_analysis/formatted_for_coloc/"
dir.create(outDir)

# test data
GWAS_folder = as.character("/lustre/scratch123/hgi/teams/trynka/resources/summary_statistics/public/GCST90012877/harmonised") 
...
# GWAS data
GWAS_path = list.files(GWAS_folder,pattern = "*.h.tsv.gz",full.names = TRUE) #there should be just one file with that pattern

# GWAS data
gwas = readr::read_tsv(GWAS_path) %>%
  dplyr::select(hm_variant_id, hm_beta,p_value,hm_effect_allele, hm_chrom,hm_pos,standard_error,effect_allele_frequency) %>%
...
GWAS_lead_variants_path = list.files(as.character(paste0(GWAS_folder,"/..")),pattern = "*GRCh38_loci*",full.names = TRUE)

##### separate this into a script that prepares the lead variants file ####
############
############
############
...
for(comp in unique(eqtl_full$group)){
  ...
  eqtl = eqtl_full %>%
    ...
    dplyr::select(chr, pos,gene_id, symbol, variant_id , beta,varbeta,pval_nominal, qval) %>%
  ...
  for (n_locus in 1:nrow(gwas_lead)) {
    ...
    subset_eqtl[[locus]] = eqtl %>%
      dplyr::group_by(symbol) %>% # group by eQTL locus and extract the lead variants that reach significance
    ...
    if (nrow(subset_eqtl[[locus]]) == 0) {
      ...
      names(sign_egenes) = subset_eqtl[[locus]]$symbol
      ####
      ...
      subset_eqtl[[locus]] = subset_eqtl[[locus]] %>%
        ...
        dplyr::mutate(symbol = names(sign_egenes[match(gene_id, sign_egenes)])) %>%
        dplyr::left_join(eqtl[c("variant_id", "qval", "symbol")],
                         by = c("variant_id", "symbol"),
                         keep = TRUE) %>%
        dplyr::rename(
        ...
        dplyr::select(!c("variant_id.y", "symbol.y", "qval.x"))
      ...
      if (nrow(subset_eqtl[[locus]]) == 0) {
        ...
        for (n in 1:length(subset_eqtl_split[[locus]])) {
          gene_symbols = append(gene_symbols, unique(subset_eqtl_split[[locus]][[n]]$symbol))
        }
        names(subset_eqtl_split[[locus]]) = gene_symbols
        
        # check which dataframe is bigger, GWAS or eQTL, then subset to shared variants
        for (n in names(subset_eqtl_split[[locus]])) {
          if (nrow(subset_eqtl_split[[locus]][[n]]) > nrow(subset_gwas[[locus]])) {
            subset_gwas[[locus]] = subset_gwas[[locus]] %>%
          ...
          tryCatch({
            check_rows(subset_gwas[[locus]], subset_eqtl_split[[locus]][[n]])
          }, error = function(e) {
          ...
          message(
            "There are ",
            sum(
              subset_gwas[[locus]]$variant_id %in% unique(subset_eqtl_split[[locus]][[n]]$variant_id)
            ),
  ...
  coloc_res[[comp]] = list()
  for(locus in names(subset_gwas)){
    ...
    for(gene in unique(subset_eqtl[[locus]]$gene_id)){
      ...
      if(nrow(subset_gwas_variants) == nrow(subset_eqt_at_gene)){
        ...
        message("WARNING: Checking and removing duplicated variants")
        subset_gwas_variants = subset_gwas_variants %>%
          dplyr::filter(!duplicated(variant_id))
        subset_eqt_at_gene = subset_eqt_at_gene %>%
          dplyr::filter(!duplicated(variant_id))
...
coloc_res_summary %>%
  ...
  readr::write_tsv(.,paste0(outDir,"/coloc_summary_single_causal_variant_",distance_threshold/1000,"kb.txt"))

readr::write_rds(coloc_res,paste0(outDir,"/coloc_results_single_causal_variant_",distance_threshold/1000,"kb.rds"))
