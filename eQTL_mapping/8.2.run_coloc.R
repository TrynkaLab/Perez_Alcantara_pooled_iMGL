...
# test data
  external_data_folder = as.character("../../data/results/8.colocalisation_analysis/subsets_for_coloc/GCST90012877/")
distance_threshold = 500000

external_data_id = basename(external_data_folder)
if(str_detect(external_data_folder,"primary_eQTLs")){
  external_data_files = list.files(external_data_folder,pattern = "*external_eQTL_subsets.rds")
  
} else {
  external_data_files = list.files(external_data_folder,pattern = "*GWAS_subsets.rds")
...
eqtl = list()
if(str_detect(external_data_folder,"primary_eQTLs")){
  for(comp in gsub(x = external_data_files,pattern = "_external_eQTL_subsets.rds",replacement = "")){
  external_data[[comp]] = readr::read_rds(file = paste0(external_data_folder,"/",comp,"_external_eQTL_subsets.rds"))
  eqtl[[comp]] = readr::read_rds(file = paste0(external_data_folder,"/",comp,"_eQTL_subsets.rds"))
  
  }
}else{
  for(comp in gsub(x = external_data_files,pattern = "_GWAS_subsets.rds",replacement = "")){
    external_data[[comp]] = readr::read_rds(file = paste0(external_data_folder,"/",comp,"_GWAS_subsets.rds"))
    eqtl[[comp]] = readr::read_rds(file = paste0(external_data_folder,"/",comp,"_eQTL_subsets.rds"))
...
for(locus in names(external_data[[comp]])){
  ...
  for(gene in names(external_data[[comp]][[locus]])){
    coloc_res[[comp]][[locus]][[gene]] = list()
    
 
  message("Building coloc input around lead external_data variant at locus ", locus)
  message("For eQTL at locus ", locus, " for variants tested for significant eGene ", gene)
  ...
  eqtl[[comp]][[locus]][[gene]] = eqtl[[comp]][[locus]][[gene]] %>%
    distinct(variant_id,.keep_all = TRUE)
  
  #test per significant eQTL gene
    subset_eqtl_coloc = list(beta = eqtl[[comp]][[locus]][[gene]]$beta,
                             varbeta = eqtl[[comp]][[locus]][[gene]]$varbeta,
                             snp = eqtl[[comp]][[locus]][[gene]]$variant_id,
                             position=eqtl[[comp]][[locus]][[gene]]$pos,
                             type="quant",
                             sdY= 1 
    )
    if(is.null(coloc::check_dataset(subset_eqtl_coloc)) & length(eqtl[[comp]][[locus]][[gene]]$variant_id)>50){
    ...
    if(nrow( external_data[[comp]][[locus]][[gene]]) == nrow(eqtl[[comp]][[locus]][[gene]])){
      message("eQTL and external_data subsets have the same number of variants")
    } else{
      message("WARNING: eQTL and external_data subsets DO NOT have the same number of variants")
      message("WARNING: Checking and removing duplicated and extra variants")
      external_data[[comp]][[locus]][[gene]] = external_data[[comp]][[locus]][[gene]] %>%
    ...
    if(str_detect(external_data_folder,"primary_eQTLs")){
      subset_external_data_coloc = list(beta = external_data[[comp]][[locus]][[gene]]$beta,
                                        varbeta = external_data[[comp]][[locus]][[gene]]$varbeta,
                                        snp = external_data[[comp]][[locus]][[gene]]$variant_id,
                                        position=external_data[[comp]][[locus]][[gene]]$pos,
                                        type="quant",
                                        sdY= 1 
      )
    } else{ # for case-control GWAS
      # double-check all the GWAS I use are like this
      subset_external_data_coloc = list(beta = external_data[[comp]][[locus]][[gene]]$beta,
                                        varbeta = external_data[[comp]][[locus]][[gene]]$varbeta,
                                        snp = external_data[[comp]][[locus]][[gene]]$variant_id,
                                        position=external_data[[comp]][[locus]][[gene]]$pos,
    ...
    if(is.null(check_dataset(subset_external_data_coloc)) & length(external_data[[comp]][[locus]][[gene]]$variant_id)>50){
    ...
    if(is.null(coloc::check_dataset(subset_external_data_coloc)) & length(external_data[[comp]][[locus]][[gene]]$variant_id)>50 & is.null(coloc::check_dataset(subset_eqtl_coloc)) & length(eqtl[[comp]][[locus]][[gene]]$variant_id)>50 ){
      coloc_res[[comp]][[locus]][[gene]]= coloc.abf(dataset1=subset_external_data_coloc,
  ...
  for(nam in names(coloc_res[[comp]] )){
    if(length(coloc_res[[comp]][[nam]])==0){
      ...
      for(gene in names(coloc_res[[comp]][[nam]])){
        coloc_res_summary[[comp]][[nam]][[gene]] = data.frame(external_data_Locus = nam,
                                                              eGene = gene, 
