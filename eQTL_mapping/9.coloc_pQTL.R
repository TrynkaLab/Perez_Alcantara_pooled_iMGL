...
outDir = "../../data/results/9.coloc_eQTL/"
dir.create(outDir)
...
# test data
pQTL_nominal_path = as.character("../../data/results/5.proportion_QTL/all_res_pqtl_migration.csv") 
eQTL_path = as.character("../../../OTAR2065_sc_eQTL/data/results/4.Inspect_eQTL_results/tensorQTL_variant_gene_60PCs.csv")
# nominal for untreated
eQTL_nominal_path=as.character("../../../OTAR2065_sc_eQTL/data/results/tensorqtl/60/sum_sizefactorsNorm_log2_scaled_centered_Not_proliferating_untreated_common_500kb_window_tensorQTL_nominal.txt")
distance_threshold = 500000

# pQTL nominal data - per treatment
pqtl = list()
for(treatment in c("untreated","IFN","LPS")){
  pqtl[[treatment]] = readr::read_csv(pQTL_nominal_path) %>%
    dplyr::select(c(snp,ends_with(treatment))) %>%
    dplyr::mutate(effect_allele = stringr::str_split(snp,pattern = "_") %>%
                    map_chr(~ tail(.x, 1))) %>%
    dplyr::rename(variant_id = snp,
                  beta = colnames(.)[2],
                  pval_nominal = colnames(.)[3]) %>%
    dplyr::mutate(chr = str_split(variant_id, "_") %>% map_chr(~ .x[1]),
                  pos = str_split(variant_id, "_") %>% map_chr(~ .x[2])) %>%
    dplyr::mutate(chr = as.numeric(chr),
                  pos=as.numeric(pos), # ensuring types are correct
