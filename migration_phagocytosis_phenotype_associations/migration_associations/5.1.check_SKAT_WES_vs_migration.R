# examine migration SKATO tests
library(patchwork)
library(tidyverse)
treatment_cols =  c(untreated = "#8D918B", IFN = "#3A5683", LPS = "#F8766D")
source("../functions.R")
directory = "../../../data/results/migration/5.1.check_SKAT_WES_vs_migration/"
...
for(mut_type in c("missense_non_deleterious","deleterious")){
  ...
  res = list()
  files = list.files("../../../data/results/migration/5.rare_var_SKAT_WES/",
  ...
  res = lapply(paste0("../../../data/results/migration/5.rare_var_SKAT_WES/",files), function(x){
    read_rds(x) 
  })
  names(res) = numbers
...
pdf(paste0(directory,"QQ_plot_migration_SKAT_pvals.pdf"),
    width = 8, height = 4 )
...
sign = res_per_treat %>%
  ...
  pdf(paste0(directory,mut_type,"_SKATO_vars_vs_phago.pdf"),
      width = 10, height = 4)
