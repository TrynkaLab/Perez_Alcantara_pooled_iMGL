...
#-------------------------------------------------------------------------------
#  Code to run within-treatment DGE for cell line migration fractions.
# --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --
...
outdir = paste(getwd(), "output_data", "06_Diff_expr_migration_permutations", "01_run_migration_DGE", sep = "/")
plotdir = paste(getwd(), "plots", "06_Diff_expr_migration_permutations", "01_run_migration_DGE", sep = "/")
dir.create(outdir, recursive = T)
dir.create(plotdir, recursive = T)

## Input dir
input_dir00 = paste(getwd(), "output_data", "01_Diff_expr_PRS_permutations", "00_Data_exploration_processing", sep = "/")
input_dir01 = paste(getwd(), "output_data", "06_Diff_expr_migration_permutations", "00_Data_exploration_processing", sep = "/")
...
## Add new ones
var_colors["migration_mean_log_fraction"] <- "violet"
var_colors["migration_mean_min_prop"] <-  "turquoise4"
...
load(paste0(input_dir01, "/rse_non_prolif_filt_migration_IFN.Rdata"), verbose = T)
load(paste0(input_dir01, "/rse_non_prolif_filt_migration_LPS.Rdata"), verbose = T)
load(paste0(input_dir01, "/rse_non_prolif_filt_migration_untreated.Rdata"), verbose = T)




## Run DGE within each treatment 
run_migration_DEG <- function(treatment){
  
  rse <- eval(parse_expr(paste("rse_non_prolif_filt_migration", treatment, sep = "_")))
  
  message(paste(dim(rse)[2], treatment, "samples across", length(unique(rse$line)), "unique lines of", length(unique(rse$donor)), "donors"))
  # IFN: 123 IFN samples across 100 unique lines of 97 donors
  # LPS: 199 LPS samples across 156 unique lines of 154 donors
  # untreated: 211 untreated samples across 157 unique lines of 154 donors
  ...
  # IFN: 12757 genes out of 18648 with >0.1 CPM in at least 30% of samples
  # LPS: 12823 genes out of 18648 with >0.1 CPM in at least 30% of samples
  # Untreated: 13121 genes out of 18648 with >0.1 CPM in at least 30% of samples
  ...
  formula <- ~ migration_mean_scaled_log_fraction + Sex + migration_mean_min_prop + genotype_PC1 + genotype_PC2
  ...
  top_genes = topTable(eBGene, coef = "migration_mean_scaled_log_fraction", p.value = 1, number = nrow(rse_expr), sort.by="none")
...
## IFN DGE
############
migration_DGE_results_IFN <- run_migration_DEG("IFN")
# "8 DEGs: 3 up-regulated and 5 down-regulated"

rse_non_prolif_filt_expr_migration_IFN <- migration_DGE_results_IFN[[1]]
save(rse_non_prolif_filt_expr_migration_IFN, file = paste0(outdir, "/rse_non_prolif_filt_expr_migration_IFN.Rdata"))
top_genes_migration_IFN <- migration_DGE_results_IFN[[2]]
save(top_genes_migration_IFN, file = paste0(outdir, "/top_genes_migration_IFN.Rdata"))
de_genes_migration_IFN <- migration_DGE_results_IFN[[3]]
save(de_genes_migration_IFN, file = paste0(outdir, "/de_genes_migration_IFN.Rdata"))

############
## LPS DGE
############
migration_DGE_results_LPS <- run_migration_DEG("LPS")
# "3072 DEGs: 1491 up-regulated and 1581 down-regulated"

rse_non_prolif_filt_expr_migration_LPS <- migration_DGE_results_LPS[[1]]
save(rse_non_prolif_filt_expr_migration_LPS, file = paste0(outdir, "/rse_non_prolif_filt_expr_migration_LPS.Rdata"))
top_genes_migration_LPS <- migration_DGE_results_LPS[[2]]
save(top_genes_migration_LPS, file = paste0(outdir, "/top_genes_migration_LPS.Rdata"))
de_genes_migration_LPS <- migration_DGE_results_LPS[[3]]
save(de_genes_migration_LPS, file = paste0(outdir, "/de_genes_migration_LPS.Rdata"))

#################
## untreated DGE
##################
migration_DGE_results_untreated <- run_migration_DEG("untreated")
# "566 DEGs: 149 up-regulated and 417 down-regulated"

rse_non_prolif_filt_expr_migration_untreated <- migration_DGE_results_untreated[[1]]
save(rse_non_prolif_filt_expr_migration_untreated, file = paste0(outdir, "/rse_non_prolif_filt_expr_migration_untreated.Rdata"))
top_genes_migration_untreated <- migration_DGE_results_untreated[[2]]
save(top_genes_migration_untreated, file = paste0(outdir, "/top_genes_migration_untreated.Rdata"))
de_genes_migration_untreated <- migration_DGE_results_untreated[[3]]
save(de_genes_migration_untreated, file = paste0(outdir, "/de_genes_migration_untreated.Rdata"))




## Compare migration log-fractions across treatments
data <- rbind(cbind(colData(rse_non_prolif_filt_expr_migration_IFN)[, c("sample_id", "migration_mean_scaled_log_fraction")], "treatment" = "IFN"), 
              cbind(colData(rse_non_prolif_filt_expr_migration_LPS)[, c("sample_id", "migration_mean_scaled_log_fraction")], "treatment" = "LPS"),
              cbind(colData(rse_non_prolif_filt_expr_migration_untreated)[, c("sample_id", "migration_mean_scaled_log_fraction")], "treatment" = "untreated"))

set.seed(01152025)
plot <- ggplot(data = data, mapping = aes(x = treatment,
                                          y = migration_mean_scaled_log_fraction,
                                          color = treatment)) +
  ...
  labs(x = "Treatment", y = "Migration scaled log-fraction") +
  theme_bw() +
...
ggsave(filename = paste0(plotdir, "/migration_log_fractions_per_treatment.pdf"), width = 3, height = 3)
...
plots_DEGs <- function(treatment){
  ...
  results <-  eval(parse_expr(paste0("top_genes_migration_", treatment)))
  ...
  p <-  ggplot(data = results,
    ...
    ggtitle(paste0("DGE for migration in ", treatment)) +
...
t_vs_t <- function(treatment1, treatment2){
  ...
  results_treatment1 <-  eval(parse_expr(paste0("top_genes_migration_", treatment1)))
  results_treatment2 <-  eval(parse_expr(paste0("top_genes_migration_", treatment2)))
  ...
  treat1_Down_treat2_Up_symbol <- subset(merged_results[order(merged_results$adj.P.Val_treatment1, decreasing = F), ], signif == paste0(treatment1, " Down; ", treatment2, " Up"))$symbol
  treat1_Up_treat2_Down_symbol <- subset(merged_results[order(merged_results$adj.P.Val_treatment1, decreasing = F), ], signif == paste0(treatment1, " Up; ", treatment2, " Down"))$symbol
...
## CCA: explore corr between covariates and migration log-fractions 
hs <- list()
for(treatment in c("IFN", "LPS", "untreated")){
  
  rse <-  eval(parse_expr(paste0("rse_non_prolif_filt_expr_migration_", treatment)))
  formula <- ~ migration_mean_scaled_log_fraction + Sex + migration_mean_min_prop + genotype_PC1 + genotype_PC2 + (1|pool)
...
for(treatment in c("IFN", "LPS", "untreated")){
  ...
  ## Covariates
  formula <- ~ migration_mean_scaled_log_fraction + migration_mean_min_prop + genotype_PC1 + genotype_PC2 + (1|Sex) + (1|pool)
