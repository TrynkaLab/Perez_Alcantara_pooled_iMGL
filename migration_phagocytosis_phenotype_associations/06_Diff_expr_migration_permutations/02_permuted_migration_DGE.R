...
#-------------------------------------------------------------------------------
#  Code to run within-treatment DGE on permuted cell line migration 
#  fractions to assess the significance of migration associations with
#  gene expr.
# --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --
...
outdir = paste(getwd(), "output_data", "06_Diff_expr_migration_permutations", "02_permuted_migration_DGE", sep = "/")
plotdir = paste(getwd(), "plots", "06_Diff_expr_migration_permutations", "02_permuted_migration_DGE", sep = "/")
...
for(treatment in c("IFN", "LPS", "untreated")){
  ...
  rse <- eval(parse_expr(paste("rse_non_prolif_filt_expr_migration", treatment, sep = "_")))
  
  ## Add TMM norm factors for voom normalization
  rse_norm <- calcNormFactors(rse, method = "TMM")
  
  ## DGE for permuted scaled log-fractions 
  coef <- paste0("permutation_migration_mean_scaled_log_fraction_", index)
  formula <- as.formula(paste("~", coef, "+ Sex + migration_mean_min_prop + genotype_PC1 + genotype_PC2"))
...
#  broom                  1.0.5      2023-06-09 [90m[1] CRAN (R 4.3.1)
...
#  glue                   1.7.0      2024-01-09 [1] [90mCRAN (R 4.3.1)
...
#  png                    0.1-8      2022-11-29 [1] CRAN (R 4.3.1)[39m
