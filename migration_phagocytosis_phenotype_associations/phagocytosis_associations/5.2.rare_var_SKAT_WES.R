...
for(mut_type in c("missense_non_deleterious","deleterious")){
  ...
  for(treat in c("untreated","LPS","IFN")){
    ...
    X = as.matrix(data.frame(intersect = 0,
                              #pool=meta_subset$pool, # I can't include pool because every element of matrix needs to be same type
    ...
    if(dim(vars_within_mut_type)[1]<2 | check_min_lines == FALSE  ){
      ...
      Get_Resampling_Pvalue(skat_res[[gene_id]])
      } else {
        obj=SKAT_Null_Model_ChrX(y.c ~ X, out_type="C",
