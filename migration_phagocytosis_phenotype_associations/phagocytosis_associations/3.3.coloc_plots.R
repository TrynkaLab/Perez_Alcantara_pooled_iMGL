...
dirs = list.dirs("../../../data/results/phagocytosis/2.check_association_results/lm_1pct_filtered_deflated/coloc_results")[-1]
...
for(outDir in dirs){
  ...
  for(nms in names(coloc_res)){
    ...
    for(locus in names(coloc_res[[nms]])){
        ...
        message("Working on ", nms, " ", locus)
        # color palette
        Pal = colorRampPalette(c('white','blue'))
        
        toplot= coloc_res[[nms]][[locus]]$results %>%
          dplyr::mutate(GWAS =  - ( pnorm(-abs(z.df1),log.p=TRUE) + log(2) ) / log(10),
                        my_GWAS =  - ( pnorm(-abs(z.df2),log.p=TRUE) + log(2) ) / log(10)) %>%
          ...
          dplyr::select(snp,position,GWAS,my_GWAS,SNP.PP.H4) %>%
        ...
        manhattan_plot = ggplot(toplot,aes(x=position,y = minus_logp)) + 
          ...
          ggtitle(paste0(locus, " locus ",
                  ...
                  subtitle = paste0("PP for colocalization = ", 
                                    floor(coloc_res[[nms]][[locus]]$summary[[6]] * 100), "%")) + 
          ylab("-log10(p-value)")+
